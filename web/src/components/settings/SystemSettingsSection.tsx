...
export function SystemSettingsSection() {
  ...
  const [mainAgentProfileId, setMainAgentProfileId] = useState<string | null>(
    null,
  );
  const [mainAgentContextSource, setMainAgentContextSource] =
  ...
  const loadAgentProfiles = useAgentProfilesStore(
    (state) => state.loadProfiles,
  );
  const updateAgentProfile = useAgentProfilesStore(
    (state) => state.updateProfile,
  );
  const agentProfilesError = useAgentProfilesStore(
    ...
    (async () => {
      ...
      try {
        ...
        if (isAdmin) {
          ...
          const mainProfile = useAgentProfilesStore
            .getState()
            .profiles.find((profile) => profile.is_default);
          setMainAgentProfileId(mainProfile?.id ?? null);
          setMainAgentContextSource(
            getAgentContextSource(mainProfile?.runtime_policy),
          );
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isAdmin && mainAgentProfileId) {
        const mainProfile = useAgentProfilesStore
          .getState()
          .profiles.find((profile) => profile.id === mainAgentProfileId);
        if (
          mainProfile &&
          getAgentContextSource(mainProfile.runtime_policy) !==
            mainAgentContextSource
        ) {
          await updateAgentProfile(mainProfile.id, {
            runtime_policy: withAgentContextSource(
              mainProfile.runtime_policy,
              mainAgentContextSource,
            ),
          });
        }
      ...
      {isAdmin && (
        <div className="space-y-5 border-t border-border pt-6">
          <h3 className="text-sm font-semibold text-foreground">
            HappyClaw 主 Agent 上下文
          </h3>

          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              <Label>继承宿主机 Claude Code 配置</Label>
              <p className="mt-0.5 text-xs leading-5 text-muted-foreground">
                开启后，主 HappyClaw 会将 ~/.claude/
                作为基础上下文。该设置与工作区的运行位置相互独立：无论运行在
                Docker 还是宿主机，都会继承这份配置。
              </p>
              <p className="mt-2 text-xs leading-5 text-muted-foreground">
                HappyClaw 管理的 Skills 与 MCP
                仍会作为附加能力生效，不会被宿主机配置替换。
              </p>
              {agentProfilesError && (
                <p className="mt-2 text-xs text-error">
                  主 Agent 配置加载失败：{agentProfilesError}
                </p>
              )}
            </div>
            <Switch
              checked={mainAgentContextSource === 'host_claude'}
              onCheckedChange={(checked) =>
                setMainAgentContextSource(checked ? 'host_claude' : 'managed')
              }
              disabled={!mainAgentProfileId || !!agentProfilesError}
              aria-label="主 HappyClaw 继承宿主机 Claude Code 配置"
            />
          </div>
        </div>
      )}
        ...
        <div className="flex items-center justify-between">
          <div className="flex-1 pr-4">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              ...
              的主容器（is_home=1，folder=main）。admin 创建的其他 host
              子群组、member 容器、Docker 容器模式均不受影响。
            </p>
            <p className="text-xs text-muted-foreground mt-2">
